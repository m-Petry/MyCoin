...
const SaveBtn = ({ data }) => {
  ...
  return (
    <button
      className="border-0 cursor-pointer bg-none outline-0"
      ...
      <div className="flex flex-col border border-gray-100 rounded mt-9">
        {cryptoData ? (
            ...
            <tbody>
              {cryptoData.map((data) => {
                return (
                  <tr
                    key={data.id}
                    className="text-base text-center border-b border-gray-100 last:border-b-0 hover:bg-gray-200 "
                  >
                    ...
                    <td
                      className={
                        data.price_change_percentage_1h_in_currency > 0
                          ? "hidden py-4 text-green lg:table-cell "
                          : "hidden py-4  text-red lg:table-cell"
                      }
                    >
                      {Number(
          ...
          <div className="flex min-h-[50vh] w-full items-center justify-center">
            <div
              className="w-8 h-8 border-4 border-solid rounded-full animate-spin border-cyan border-b-gray-200"
              role="status"
            />
            <span className="ml-2 text-base">please wait...</span>
          </div>
        ) : error.data || error.search ? (
          <h1 className="flex min-h-[60vh] items-center justify-center text-lg text-red ">
            {error.data
              ? error.data
              : error.search
              ? error.search
